...
export var storeObject = {
    ...
    addDashboardListeners() {
        ...
        projects.on("child_added", (snap) => this.sync("projects", snap.key, snap.val()));
        projects.on("child_changed", (snap) => this.sync("projects", snap.key, snap.val()));
        projects.on("child_removed", (snap) => this.sync("projects", snap.key, snap.val()));
    },
    addDocumentListeners() {
        this.projectRef.child("users").on("value", (snap) => this.sync("users", "", snap.val()));
        this.projectRef.child("nodes").on("child_added", (snap) => this.sync("cards", snap.key, snap.val()));
        this.projectRef.child("nodes").on("child_changed", (snap) => this.sync("cards", snap.key, snap.val()));
        this.projectRef.child("nodes").on("child_removed", (snap) => delete this.cards[snap.key]);
        this.projectRef.child("container").on("value", (snap) => this.sync("container", "", snap.val()));
    },
    addCursorListener() {
        this.projectRef.child("cursors").on('value', (snap) => this.sync("cursors", "", snap.val()));
    },
    removeDashbardListeners() {
